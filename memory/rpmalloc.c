...
//! Default number of spans to map in call to map more virtual memory (default values yield 4MiB
//! here)
...
atomic_incr32(atomic32_t* val) {
	return (int32_t)_InterlockedExchangeAdd(val, 1) + 1;
}
...
atomic_add32(atomic32_t* val, int32_t add) {
	return (int32_t)_InterlockedExchangeAdd(val, add) + add;
}
static FORCEINLINE void*
...
atomic_cas_ptr(atomicptr_t* dst, void* val, void* ref) {
	return (_InterlockedCompareExchange64((volatile long long*)dst, (long long)val,
	                                      (long long)ref) == (long long)ref) ?
	           1 :
	           0;
}
#else
static FORCEINLINE int
atomic_cas_ptr(atomicptr_t* dst, void* val, void* ref) {
	return (_InterlockedCompareExchange((volatile long*)dst, (long)val, (long)ref) == (long)ref) ?
	           1 :
	           0;
}
#endif
...
static FORCEINLINE int
atomic_cas_ptr(atomicptr_t* dst, void* val, void* ref) {
	return atomic_compare_exchange_weak_explicit(dst, &ref, val, memory_order_release,
...
// A span can either represent a single span of memory pages with size declared by span_map_count
// configuration variable, or a set of spans in a continuous region, a super span. Any reference to
// the term "span" usually refers to both a single span or a super span. A super span can further be
// divided into multiple spans (or this, super spans), where the first (super)span is the master and
// subsequent (super)spans are subspans. The master span keeps track of how many subspans that are
// still alive and mapped in virtual memory, and once all subspans and master have been unmapped the
// entire superspan region is released and unmapped (on Windows for example, the entire superspan
// range has to be released in the same call to release the virtual memory range, but individual
// subranges can be decommitted individually to reduce physical memory use).
struct span_t {
	//! Free list
	void* free_list;
	//! State
...
struct heap_class_t {
	...
	//  Current active span is at head of list. Previous span pointer in head points to tail span of
	//  list.
	span_t* partial_span;
};
...
#define _memory_statistics_add_peak(atomic_counter, value, peak)             \
	do {                                                                     \
		int32_t _cur_count = atomic_add32(atomic_counter, (int32_t)(value)); \
		if (_cur_count > (peak))                                             \
			peak = _cur_count;                                               \
	} while (0)
#define _memory_statistics_sub(atomic_counter, value) \
	atomic_add32(atomic_counter, -(int32_t)(value))
#define _memory_statistics_inc_alloc(heap, class_idx)                                          \
	do {                                                                                       \
...
#define _memory_statistics_inc_free(heap, class_idx)                   \
	do {                                                               \
		atomic_decr32(&heap->size_class_use[class_idx].alloc_current); \
		atomic_incr32(&heap->size_class_use[class_idx].free_total);    \
	} while (0)
#else
#define _memory_statistics_inc(counter, value) \
	do {                                       \
	} while (0)
#define _memory_statistics_dec(counter, value) \
	do {                                       \
	} while (0)
...
_memory_map(size_t size, size_t* offset) {
	...
	assert(size >= _memory_page_size);
	_memory_statistics_add_peak(&_mapped_pages, (size >> _memory_page_size_shift),
	                            _mapped_pages_peak);
...
//! Get the aligned number of spans to map in based on wanted count, configured mapping granularity
//! and the page size
static size_t
_memory_map_align_span_count(size_t span_count) {
	size_t request_count =
	    (span_count > _memory_span_map_count) ? span_count : _memory_span_map_count;
...
static void
_memory_span_initialize(span_t* span, size_t total_span_count, size_t span_count,
                        size_t align_offset) {
...
_memory_map_aligned_span_count(heap_t* heap, size_t span_count) {
	// If we already have some, but not enough, reserved spans, release those to heap cache and map
	// a new full set of spans. Otherwise we would waste memory if page size > span size (huge
	// pages)
	...
	if (aligned_span_count > span_count) {
		...
		_memory_heap_set_reserved_spans(heap, span,
		                                pointer_offset(span, span_count * _memory_span_size),
...
_memory_unmap_span(span_t* span) {
	...
	span_t* master =
	    is_master ?
	        span :
	        (pointer_offset(
	            span, -(intptr_t)((uintptr_t)span->total_spans_or_distance * _memory_span_size)));
	assert(is_master || (span->flags & SPAN_FLAG_SUBSPAN));
	assert(master->flags & SPAN_FLAG_MASTER);

	size_t span_count = span->span_count;
	if (!is_master) {
		// Directly unmap subspans (unless huge pages, in which case we defer and unmap entire page
		// range with master)
	...
	if (atomic_add32(&master->remaining_spans, -(int32_t)span_count) <= 0) {
		// Everything unmapped, unmap the master span with release flag to unmap the entire range of
		// the super span
		...
		_memory_unmap(master, unmap_count * _memory_span_size, master->align_offset,
		              master->total_spans_or_distance * _memory_span_size);
	}
}
...
_memory_cache_extract(global_cache_t* cache) {
	...
	do {
		...
		if (span_ptr) {
			...
			// By accessing the span ptr before it is swapped out of list we assume that a
			// contending thread does not manage to traverse the span to being unmapped before we
			// access it
			void* new_cache =
			    (void*)((uintptr_t)span->prev |
			            ((uintptr_t)atomic_incr32(&cache->counter) & ~_memory_span_mask));
...
_memory_global_cache_insert(span_t* span) {
	...
	const size_t cache_limit =
	    (GLOBAL_CACHE_MULTIPLIER *
	     ((span_count == 1) ? _memory_span_release_count : _memory_span_release_count_large));
...
#else
	const size_t release_count =
	    (!idx ? _memory_span_release_count : _memory_span_release_count_large);
...
//! Get a span from one of the cache levels (thread cache, reserved, global cache) or fallback to
//! mapping more memory
static span_t*
...
static void*
_memory_span_set_new_active(heap_t* heap, heap_class_t* heap_class, span_t* span,
                            uint32_t class_idx) {
	...
	void* block;
	span->free_list_limit = free_list_partial_init(&heap_class->free_list, &block, span,
...
_memory_allocate_from_heap_fallback(heap_t* heap, uint32_t class_idx) {
	...
	if (EXPECTED(active_span != 0)) {
		...
		if (active_span->free_list_limit < active_span->block_count) {
			...
			active_span->free_list_limit += free_list_partial_init(
		...
		// If the active span is fully allocated, mark span as free floating (fully allocated and
...
_memory_allocate_medium(heap_t* heap, size_t size) {
	// Calculate the size class index and do a dependent lookup of the final class index (in case of
	// merged classes)
	const uint32_t base_idx = (uint32_t)(
	    SMALL_CLASS_COUNT + ((size - (SMALL_SIZE_LIMIT + 1)) >> MEDIUM_GRANULARITY_SHIFT));
...
_memory_heap_orphan(heap_t* heap) {
	...
	do {
		...
		raw_heap =
		    (void*)((uintptr_t)heap | (orphan_counter & (uintptr_t)(HEAP_ORPHAN_ABA_SIZE - 1)));
...
_memory_allocate_heap(void) {
	...
	do {
		...
		orphan_counter = (uintptr_t)atomic_incr32(&_memory_orphan_counter);
		next_raw_heap = (void*)((uintptr_t)next_heap |
		                        (orphan_counter & (uintptr_t)(HEAP_ORPHAN_ABA_SIZE - 1)));
	...
	if (!heap) {
		...
		// Put extra heaps as orphans, aligning to make sure ABA protection bits fit in pointer low
		// bits
		...
		if (span->heap) {
			if (span->size_class < SIZE_CLASS_COUNT) {
				...
				if ((size_t)span->block_size >= size) {
					// Still fits in block, never mind trying to save memory, but preserve data if
					// alignment changed
	...
	for (size_t iclass = 0; iclass < LARGE_CLASS_COUNT; ++iclass) {
		...
		while (span) {
			...
			size_t release_count =
			    (!iclass ? _memory_span_release_count : _memory_span_release_count_large);
			span_t* next = _memory_span_list_split(span, (uint32_t)release_count);
#if ENABLE_STATISTICS
			heap->thread_to_global +=
			    (size_t)span->list_size * span->span_count * _memory_span_size;
...
#if defined(_MSC_VER) && !defined(__clang__) && \
    (!defined(BUILD_DYNAMIC_LINK) || !BUILD_DYNAMIC_LINK)
#include <fibersapi.h>
...
rpmalloc_initialize_config(const rpmalloc_config_t* config) {
	...
	_memory_span_map_count =
	    (_memory_config.span_map_count ? _memory_config.span_map_count : DEFAULT_SPAN_MAP_COUNT);
	...
	_memory_span_release_count =
	    (_memory_span_map_count > 4 ?
	         ((_memory_span_map_count < 64) ? _memory_span_map_count : 64) :
	         4);
	_memory_span_release_count_large =
	    (_memory_span_release_count > 8 ? (_memory_span_release_count / 4) : 2);
...
#if ENABLE_THREAD_CACHE
			// Free span caches (other thread might have deferred after the thread using this heap
			// finalized)
			_memory_heap_cache_adopt_deferred(heap);
			for (size_t iclass = 0; iclass < LARGE_CLASS_COUNT; ++iclass) {
...
static void*
_memory_map_os(size_t size, size_t* offset) {
	// Either size is a heap (a single page) or a (multiple) span - we only need to align spans, and
	// only if larger than map granularity
	size_t padding =
	    ((size >= _memory_span_size) && (_memory_span_size > _memory_map_granularity)) ?
	        _memory_span_size :
	        0;
	assert(size >= _memory_page_size);
#if PLATFORM_WINDOWS
	// Ok to MEM_COMMIT - according to MSDN, "actual physical pages are not allocated unless/until
	// the virtual addresses are actually accessed"
	void* ptr = VirtualAlloc(0, size + padding,
	                         (_memory_huge_pages ? MEM_LARGE_PAGES : 0) | MEM_RESERVE | MEM_COMMIT,
	                         PAGE_READWRITE);
	if (!ptr) {
...
rpaligned_realloc(void* ptr, size_t alignment, size_t size, size_t oldsize, unsigned int flags) {
	...
	if (alignment > 32) {
		...
		if ((usablesize >= size) && (size >= (usablesize / 2)) &&
...
rpmalloc_thread_statistics(rpmalloc_thread_statistics_t* stats) {
	...
	for (size_t iclass = 0; iclass < LARGE_CLASS_COUNT; ++iclass) {
		if (heap->span_cache[iclass])
			stats->spancache =
			    (size_t)heap->span_cache[iclass]->list_size * (iclass + 1) * _memory_span_size;
	...
	for (size_t iclass = 0; iclass < SIZE_CLASS_COUNT; ++iclass) {
		...
		stats->size_use[iclass].spans_to_cache =
		    (size_t)heap->size_class_use[iclass].spans_to_cache;
		stats->size_use[iclass].spans_from_cache =
		    (size_t)heap->size_class_use[iclass].spans_from_cache;
		stats->size_use[iclass].spans_from_reserved =
		    (size_t)heap->size_class_use[iclass].spans_from_reserved;
...
#if ENABLE_GLOBAL_CACHE
	for (size_t iclass = 0; iclass < LARGE_CLASS_COUNT; ++iclass) {
		stats->cached += (size_t)atomic_load32(&_memory_span_cache[iclass].size) * (iclass + 1) *
		                 _memory_span_size;
	}
#endif
}
...
_memory_heap_dump_statistics(heap_t* heap, void* file) {
	...
	fprintf(file,
	        "Class   CurAlloc  PeakAlloc   TotAlloc    TotFree  BlkSize BlkCount SpansCur "
	        "SpansPeak  PeakAllocMiB  ToCacheMiB FromCacheMiB FromReserveMiB MmapCalls\n");
	for (size_t iclass = 0; iclass < SIZE_CLASS_COUNT; ++iclass) {
		if (!heap->size_class_use[iclass].alloc_total)
			continue;
		fprintf(file, "%3u:  %10u %10u %10u %10u %8u %8u %8d %9d %13zu %11zu %12zu %14zu %9u\n",
		        (uint32_t)iclass, atomic_load32(&heap->size_class_use[iclass].alloc_current),
		        heap->size_class_use[iclass].alloc_peak, heap->size_class_use[iclass].alloc_total,
		        atomic_load32(&heap->size_class_use[iclass].free_total),
		        _memory_size_class[iclass].block_size, _memory_size_class[iclass].block_count,
		        heap->size_class_use[iclass].spans_current, heap->size_class_use[iclass].spans_peak,
		        ((size_t)heap->size_class_use[iclass].alloc_peak *
		         (size_t)_memory_size_class[iclass].block_size) /
		            (size_t)(1024 * 1024),
		        ((size_t)heap->size_class_use[iclass].spans_to_cache * _memory_span_size) /
		            (size_t)(1024 * 1024),
		        ((size_t)heap->size_class_use[iclass].spans_from_cache * _memory_span_size) /
		            (size_t)(1024 * 1024),
		        ((size_t)heap->size_class_use[iclass].spans_from_reserved * _memory_span_size) /
		            (size_t)(1024 * 1024),
		        heap->size_class_use[iclass].spans_map_calls);
	}
	fprintf(file,
	        "Spans  Current     Peak  PeakMiB  Cached  ToCacheMiB FromCacheMiB ToReserveMiB "
	        "FromReserveMiB ToGlobalMiB FromGlobalMiB  MmapCalls\n");
	for (size_t iclass = 0; iclass < LARGE_CLASS_COUNT; ++iclass) {
		if (!heap->span_use[iclass].high && !heap->span_use[iclass].spans_map_calls)
			continue;
		fprintf(
		    file, "%4u: %8d %8u %8zu %7u %11zu %12zu %12zu %14zu %11zu %13zu %10u\n",
		    (uint32_t)(iclass + 1), atomic_load32(&heap->span_use[iclass].current),
		    heap->span_use[iclass].high,
		    ((size_t)heap->span_use[iclass].high * (size_t)_memory_span_size * (iclass + 1)) /
		        (size_t)(1024 * 1024),
		    heap->span_cache[iclass] ? heap->span_cache[iclass]->list_size : 0,
		    ((size_t)heap->span_use[iclass].spans_to_cache * (iclass + 1) * _memory_span_size) /
		        (size_t)(1024 * 1024),
		    ((size_t)heap->span_use[iclass].spans_from_cache * (iclass + 1) * _memory_span_size) /
		        (size_t)(1024 * 1024),
		    ((size_t)heap->span_use[iclass].spans_to_reserved * (iclass + 1) * _memory_span_size) /
		        (size_t)(1024 * 1024),
		    ((size_t)heap->span_use[iclass].spans_from_reserved * (iclass + 1) *
		     _memory_span_size) /
		        (size_t)(1024 * 1024),
		    ((size_t)heap->span_use[iclass].spans_to_global * (size_t)_memory_span_size *
		     (iclass + 1)) /
		        (size_t)(1024 * 1024),
		    ((size_t)heap->span_use[iclass].spans_from_global * (size_t)_memory_span_size *
		     (iclass + 1)) /
		        (size_t)(1024 * 1024),
		    heap->span_use[iclass].spans_map_calls);
	}
	fprintf(file, "ThreadToGlobalMiB GlobalToThreadMiB\n");
	fprintf(file, "%17zu %17zu\n", (size_t)heap->thread_to_global / (size_t)(1024 * 1024),
	        (size_t)heap->global_to_thread / (size_t)(1024 * 1024));
}
...
rpmalloc_dump_statistics(void* file) {
	...
	fprintf(file, "%9zu %11zu %13zu %14zu %16zu %16zu\n", mapped / (size_t)(1024 * 1024),
	        mapped_os / (size_t)(1024 * 1024), mapped_peak / (size_t)(1024 * 1024),
	        mapped_total / (size_t)(1024 * 1024), unmapped_total / (size_t)(1024 * 1024),

...
static FORCEINLINE int
atomic_cas_ptr_acquire(atomicptr_t* dst, void* val, void* ref) {
...
_rpmalloc_span_initialize_new(heap_t* heap, span_t* span, uint32_t class_idx) {
	...
	span->free_list_limit =
	    free_list_partial_init(&heap->free_list[class_idx], &block, span, pointer_offset(span, SPAN_HEADER_SIZE),
...
_rpmalloc_global_cache_extract(global_cache_t* cache) {
	...
	do {
		...
		if (span_ptr) {
			...
			void* new_cache =
			    (void*)((uintptr_t)span->prev | ((uintptr_t)atomic_incr32(&cache->counter) & ~_memory_span_mask));
...
_rpmalloc_global_cache_insert_span_list(span_t* span) {
	...
	const size_t cache_limit =
	    (GLOBAL_CACHE_MULTIPLIER * ((span_count == 1) ? _memory_span_release_count : _memory_span_release_count_large));
	...
	if (EXPECTED(span != 0)) {
		...
		if (span->free_list) {
			...
			span->free_list_limit += free_list_partial_init(
			    &heap->free_list[class_idx], &block, (void*)((uintptr_t)block_start & ~(_memory_page_size - 1)),
...
_rpmalloc_deallocate_defer_small_or_medium(span_t* span, void* block) {
	...
	do {
		...
		*((void**)block) = free_list;
	         ...
	         !atomic_cas_ptr_acquire(&span->free_list_deferred, INVALID_POINTER, free_list));
...
_memory_heap_dump_statistics(heap_t* heap, void* file) {
	...
	fprintf(file,
	        "Spans  Current     Peak  PeakMiB  Cached  ToCacheMiB FromCacheMiB ToReserveMiB FromReserveMiB ToGlobalMiB "

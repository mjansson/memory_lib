...
static void* _memory_malloc_from_active( memory_heap_t* heap )
{
	...
	do
	{
		...
		address = (char*)descriptor->superblock + ( ( ( old_anchor.anchor.available + 1 ) * descriptor->size ) - sizeof( void* ) );
#else
...
#if BUILD_USE_PRE_ALIGN
	descriptor->max_count = ( heap->size_class->superblock_size / descriptor->size ) - 1; //Use first block for alignment and clobber watch overhead
#else
...
static int _memory_initialize( void )
{
	...
	num_heap_pool = system_hardware_threads() + 1;
	if( num_heap_pool < 3 )
		num_heap_pool = 3;
	if( num_heap_pool > 32 )
		num_heap_pool = 32;

	size = ( sizeof( memory_heap_pool_t ) * num_heap_pool ) + ( num_sizeclass * ( sizeof( memory_sizeclass_t ) + ( sizeof( memory_heap_t ) * num_heap_pool ) ) );
...
static void _memory_shutdown( void )
{
	...
	size = ( sizeof( memory_heap_pool_t ) * _memory_num_heap_pool ) + ( _memory_num_sizeclass * ( sizeof( memory_sizeclass_t ) + ( sizeof( memory_heap_t ) * _memory_num_heap_pool ) ) );
